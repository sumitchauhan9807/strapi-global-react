		...
		<>
			...
			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
							...
							>
								<div className="rounded lg:p-5 lg:transition lg:duration-300 lg:hover:bg-indigo-50">
									<div className="flex items-center mb-1">
										<span className={`flex items-center justify-center w-4 h-4 mr-2 text-xs font-medium text-white rounded ${colorStyle()}`}>{index + 1}</span>
										<p className="text-lg font-semibold sm:text-base">{item.Heading}</p>
									</div>
									<p className="text-sm text-gray-900">{item.SubHeading}</p>
								</div>
							</Animate>
						);
					})}
				...
				<div className="relative md:col-span-2 lg:col-span-2">
					...
					>
						<img className="inset-0 object-cover object-bottom w-full  shadow-lg " src={baseUrl() + data.Image.url} alt="" />
					</Animate>
				</div>
			</div>
